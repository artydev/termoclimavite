...
Totale: €${t}`)}function x(t){u.value=t}function f(){u.value=null}function L(){try{if(n.value.length===0&&typeof localStorage<"u"){localStorage.removeItem("cart");return}const t=JSON.stringify(n.value);typeof localStorage<"u"&&localStorage.setItem("cart",t)}catch(t){console.error("Error saving cart:",t)}}y(()=>{L()});function A(){return i`
  ...
  </header>`}function F(){return i`<div class="filters">
    <button 
  ...
  </div>`}function I(t){return t.specifications&&Object.entries(t.specifications).slice(0,2),i`<div class="product">
  ...
  </div>`}function N(){const t=q.value;return s.value?i`<div class="loading">Caricamento prodotti...</div>`:t.length===0?i`<div class="loading">Nessun prodotto trovato</div>`:i`<div class="products">
    ${t.map(e=>I(e))}
  </div>`}function T(t){return i`<div class="cart-item">
  ...
  </div>`}function j(){const t=k.value;return i`<div class="cart-overlay ${p.value?"active":""}" onclick=${g}>
  ...
  </div>`}function D(){if(!u.value)return"";const t=u.value;return i`
